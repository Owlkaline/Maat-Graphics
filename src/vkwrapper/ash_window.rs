  ...
  match (display_handle, window_handle) {
    (RawDisplayHandle::Windows(_), RawWindowHandle::Win32(window)) => {
      let surface_desc = vk::Win32SurfaceCreateInfoKHR::builder()
        .hinstance(isize::from(window.hinstance.unwrap()) as *const c_void)
        .hwnd(isize::from(window.hwnd) as *const c_void);
    ...
    (RawDisplayHandle::Xlib(display), RawWindowHandle::Xlib(window)) => {
      let surface_desc = vk::XlibSurfaceCreateInfoKHR::builder()
        .dpy(display.display.unwrap().as_ptr() as _)
    ...
    (RawDisplayHandle::Xcb(display), RawWindowHandle::Xcb(window)) => {
      let surface_desc = vk::XcbSurfaceCreateInfoKHR::builder()
        .connection(display.connection.unwrap().as_ptr() as _)
